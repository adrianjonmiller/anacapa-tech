...
function wpv_admin_archive_listing_page() {
	...
	<div class="wrap toolset-views">
		...
		if ( isset( $_GET['trashed'] ) && is_numeric( $_GET['trashed'] ) ) { ?>
			<p class="toolset-alert toolset-alert-warning update below-h2">
				<?php _e('WordPress Archive moved to the Trash', 'wpv-views'); ?> <button class="button-secondary js-wpv-untrash" data-id="<?php echo $_GET['trashed']; ?>" data-nonce="<?php echo wp_create_nonce( 'wpv_view_listing_actions_nonce' ); ?>"><?php _e('Undo', 'wpv-views'); ?></button>
			</p>
		<?php }
