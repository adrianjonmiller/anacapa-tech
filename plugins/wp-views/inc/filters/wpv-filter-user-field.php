...
if(is_admin()){
	...
	function get_user_meta_keys($include_hidden = false) {
		...
		if ( !$include_hidden ) {
			...
			'capabilities','wp_capabilities','user_level','wp_user_level','dismissed_wp_pointers','show_welcome_panel',
			'dashboard_quick_press_last_post_id','wp_dashboard_quick_press_last_post_id','managenav-menuscolumnshidden',
			'metaboxhidden_nav-menus','closedpostboxes_nav-menus','nav_menu_recently_edited',
			'primary_blog','source_domain','user-settings-time','meta-box-order_dashboard',
			'user-settings','wp_user-settings','user-settings-time','wp_user-settings-time',
			'closedpostboxes_post','metaboxhidden_post','source_domain',
			'new_date','show_highlight','language_pairs',
			'wp_module-manager','wp_module-manager-time',
			'closedpostboxes','meta-box-order','metaboxhidden','screen_layout');
			$umf_hidden = " ( meta_key NOT REGEXP '" . implode("|", $hidden_usermeta) . "' AND meta_key NOT REGEXP '^_' ) ";
		}
		
		$where = " WHERE {$umf_mulsitise_string} AND {$umf_hidden} ";
		
		$usermeta_keys = array();
		$usermeta_keys = $wpdb->get_col( $wpdb->prepare(
			"
        ...
        if ($cf_keys == null) {
            ...
            $cf_keys = $wpdb->get_col( "SELECT distinct $wpdb->usermeta.meta_key FROM $wpdb->usermeta" );
            if ( is_multisite() ) {
			...
			if (!$include_hidden) {
				...
				$exclude_these_hidden = array('comment_shortcuts','managenav-menuscolumnshidden','dismissed_wp_pointers','meta-box-order_dashboard','nav_menu_recently_edited',
            ...
            ,'language_pairs','first_name','last_name','name','nickname','description','yim','jabber','aim');
				$exclude_these_hidden_var = '/('.implode('|', $exclude_these_hidden).')/';
				...
				foreach ($cf_keys as $index => $field) {
					...
					if ( preg_match($exclude_these_hidden_var, $field) ){
							unset($cf_keys[$index]);
					}
					
					
					if ( preg_match("/(closedpostboxes|meta-box-order|metaboxhidden|screen_layout)/", $field) ){
						unset($cf_keys[$index]);
					}
					if ( is_multisite() ) {
						 foreach ($blogs as $blog) {
						 	if ( strpos( $field, $wpdb->base_prefix.$blog->blog_id.'_') === 0 && $blog_id !== $blog->blog_id){
								unset($cf_keys[$index]);
							}
						 }	
					}
